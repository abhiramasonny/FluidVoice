...
struct ContentView: View {
    ...
    private func stopAndProcessTranscription(route: DictationOutputRoute = .normal) async {
        ...
        // Keep the overlay visible until delivery posts Cmd+V or reports a recoverable failure.
        await Task.yield()
        ...
        if shouldTypeExternally {
            ...
            } else if !shouldShowAIProcessingFailure {
        ...
        if !didTypeExternally, !shouldShowAIProcessingFailure, !didFailTextDelivery {
            self.hideOverlayAfterOutput()
        }
    }
